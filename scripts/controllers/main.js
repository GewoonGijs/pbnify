...
angular.module('pbnApp')
  .controller('MainCtrl', function ($scope) {
      ...
      $scope.addCustomColor = function() {
        $scope.palette.push({x:'', y:'',
                              r:parseInt($scope.nonInput.color[1]+$scope.nonInput.color[2], 16),
                              g:parseInt($scope.nonInput.color[3]+$scope.nonInput.color[4], 16),
                              b:parseInt($scope.nonInput.color[5]+$scope.nonInput.color[6], 16) });
      }
      ...
      $scope.swapPalette = function() {
        for (var i in $scope.palette) {
          $scope.palette[i].r = parseInt($scope.alt_palette[i].color[1]+ $scope.alt_palette[i].color[2], 16);
          $scope.palette[i].g = parseInt($scope.alt_palette[i].color[3]+ $scope.alt_palette[i].color[4], 16);
          $scope.palette[i].b = parseInt($scope.alt_palette[i].color[5]+ $scope.alt_palette[i].color[6], 16);
          $scope.alt_palette[i].color = $scope.palette[i].hex;
        }
        ...
        var imgData = matToImageData($scope.matSmooth, $scope.palette, ctx2);
        ctx2.putImageData(imgData, 0, 0);
      };

      $scope.autoPalette = function() {
        $scope.palette=[];
        var pal = extractColourPalette($scope.c, $scope.num.value);
        for (var clr in pal) {
          $scope.palette.push({x:'', y:'', r:Math.floor(pal[clr].r), g:Math.floor(pal[clr].g), b:Math.floor(pal[clr].b)})
        }
      }
      ...
      $scope.pbnify = function() {
  	  ...
  	  worker.addEventListener('message', function(e) {
  		  ...
  		  $scope.matSmooth = e.data.matSmooth;
  		  var labelLocs = e.data.labelLocs;
  		  var matLine = e.data.matLine;
  		  worker.terminate();

  		  displayResults($scope.matSmooth, matLine, labelLocs);
		    getColorInfo($scope.palette);  // adds hex and CMYK values for display
        $scope.alt_palette = [];
        for (var i in  $scope.palette){
          $scope.alt_palette.push({ color: $scope.palette[i].hex});
        }
        $scope.step = "result";
  		  $scope.view = "filled";
  		  $scope.$apply();
